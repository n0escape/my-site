...
function gamestart(){
    const Promise1 = new Promise(function(resolve){
        ...
        function soundBtn() {
            if(video.muted){
                video.muted = false;
                sound.setAttribute('src', 'volume.png')
            } else {
                video.muted = true;
                    ...
                    setTimeout(()=>{
                        ...
                        function displayStatusText(ctx){
                            ...
                            if(gameOver){
                                ...
                                ctx.textAlign = 'center';
                                ctx.fillStyle = 'white';
                                ctx.fillText('Game over, press Space to restart, time to win: ' + score, canvas.width/2, 250, canvas.width - 20);
                            }
                            if(win){
                                ctx.fillStyle = '#333';
                                ...
                                ctx.fillStyle = 'white';
                                ctx.fillRect(this.x, this.y, this.width, this.height);
                                ctx.fillText('Victory! Congratulations!', canvas.width/2, 250);
                            }
                            if (pause){
                                pauseTab();
                            }
                                ...
                                setTimeout(()=>{
                                    ...
                                    function displayStatusText(ctx){
                                        ...
                                        if(gameOver){
                                            ...
                                            ctx.fillStyle = 'white';
                                            ctx.fillRect(this.x, this.y, this.width, this.height);
                                            ctx.fillText('Game over, press Space to restart, your score: ' + score, canvas.width/2, 250, canvas.width - 20);
                                        }
                                        if(win){
                                            ctx.fillStyle = '#333';
                                            ctx.fillRect(0, 0, canvas.width, canvas.height);
                                            ...
                                            const Promise6 = new Promise(function(resolve){
                                                setTimeout(()=>{
                                                    ...
                                                    function displayStatusText(ctx){
                                                        ...
                                                        if (pause){
                                                            ctx.fillStyle = 'white';
                                                            ctx.fillText('Pause', canvas.width/2, 250);
                                                        }
                                                        if(pause){
                                                            pauseTab();
                                                        }
                                                                ...
                                                                setTimeout(()=>{
                                                                    ...
                                                                    class Background {
                                                                        ...
                                                                        draw(ctx){
                                                                            ...
                                                                            ctx.fillRect(this.x, this.y - canvas.height, canvas.width, canvas.height);
                                                                        }
                                                                        update(){
                                                                            this.y += this.speed;
                                                                    ...
                                                                    class Planet {
                                                                        ...
                                                                        draw(ctx){
                                                                            ctx.beginPath();
                                                                            ctx.arc(this.x + this.radius, this.y, this.radius, 0, Math.PI, false);
                                                                            ctx.fillStyle = "yellow";
                                                                            ctx.fill();
                                                                            ctx.lineWidth = 1;
                                                                            ctx.strokeStyle = 'red';
                                                                    ...
                                                                    function displayStatusText(ctx){
                                                                        ...
                                                                        ctx.textAlign = 'left';
                                                                        ctx.fillText('Fuel: ' + score, 20, 50);

                                                                        ctx.textAlign = 'center';
                                                                        ctx.fillText('Turbo: ' + turbo + '%',(canvas.width/2) + 20, 50);

                                                                        ctx.textAlign = 'right';
                                                                        ctx.fillText('Time: ' + timer, canvas.width - 20, 50);
                                                                        ...
                                                                        if(gameOver){
                                                                            ...
                                                                            ctx.textAlign = 'center';
                                                                            ctx.fillStyle = 'white';
                                                                            ctx.fillText('Game over, press Space to restart, time to win: ' + timer, canvas.width/2, 250, canvas.width - 20);
                                                                        }
                                                                        if(win){
                                                                            ctx.fillStyle = '#333';
                                                                            ctx.fillRect(0, 0, canvas.width, canvas.height);
...
function rule1(){
    ...
    ctx.fillText("Enter - возобновить игру", canvas.width/2, 740 );
...
function rule2(){
    ...
    ctx.fillText("Правила игры 2", canvas.width/2, 130, canvas.width-100 );
    ctx.fillText("Игра длиться до 100 очков", canvas.width/2, 170 );
    ctx.fillText("Цель игры заключается в том чтобы избежать летающих чудищ и добраться до врат", canvas.width/2, 210, canvas.width-100 );
    ctx.fillText("По достижении отметки в 100 очков, появляются врата окончания игры", canvas.width/2, 250, canvas.width-100 );

    ctx.fillStyle = 'red';
    ctx.fillText("Важно!", canvas.width/2, 340 );
    ctx.fillStyle = 'black';
    ctx.fillText("Очки зачисляются лишь тогда, когда враг ушел за пределы карты", canvas.width/2, 380, canvas.width-100 );
...
function rule3(){
    ...
    ctx.fillText("Правила игры 3", canvas.width/2, 130, canvas.width-100 );
    ctx.fillText("Игра длиться до нахождения портала", canvas.width/2, 170 );
    ctx.fillText("Цель игры заключается в том чтобы найти правильный путь в лабиринте", canvas.width/2, 210, canvas.width-100 );
...
function rule4(){
    ...
    ctx.fillText("Правила игры 4", canvas.width/2, 130 );
    ctx.fillText("Цель игры заключается в том чтобы избежать астероидов и высадится на безопасной планете", canvas.width/2, 170, canvas.width-100 );
    ctx.fillText("Чтобы долететь, попутно надо собирать баночки с топливом (синие квадратики)", canvas.width/2, 210, canvas.width-100 );
    ctx.fillText("По истечении отсчета времени (справа сверху), появится планета, куда надо долететь", canvas.width/2, 250, canvas.width-100 );

    ctx.fillStyle = 'red';
    ctx.fillText("Важно!", canvas.width/2, 290 );
    ctx.fillStyle = 'black';
    ctx.fillText("1) Чтобы использовать турбо скачок, нужно чтобы показатель турбо был равен 100%", canvas.width/2, 330 );
    ctx.fillText("2) Вы в космосе поэтому переодически добавляйте газу чтобы лететь, иначе СМЭРТЬ", canvas.width/2, 370 );
    ctx.fillText("3) Следите за уровнем топлива, иначе СМЭРТЬ", canvas.width/2, 410 );
    ctx.fillText("4) Следите также чтобы ракета не вылетела за нижнюю границу, иначе СМЭРТЬ", canvas.width/2, 450 );

    ctx.fillText("Управление", canvas.width/2, 490 );
    ctx.fillText("W - полет вперед / добавить газку", canvas.width/2, 530 );
    ctx.fillText("S - движение влево / сбавить обороты", canvas.width/2, 570 );
    ctx.fillText("A - движение влево", canvas.width/2, 610 );
    ctx.fillText("D - движение вправо", canvas.width/2, 650 );
    ctx.fillText("L Shift - пространсвенный скачек вперед", canvas.width/2, 690 );
    ctx.fillText("Space - перезапуск уровня", canvas.width/2, 730 );
    ctx.fillText("Escape - пауза", canvas.width/2, 770 );
    ctx.fillText("Enter - возобновить игру", canvas.width/2, 810 );
}
