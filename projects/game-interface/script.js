...
//            context.drawImage(this.image, this.x, this.y, canvas.width, canvas.height);
    ...
    function callRule(){
        ...
        let resume = new Button( (canvas.width/2) - 50, (canvas.width/2) * 1.6, 100, 50, 'blue' );

        resume.draw(ctx);
        ctx.fillStyle = 'black';
        ctx.fillText('Resume', canvas.width/2, ((canvas.width/2) * 1.6) + 30 );
    ...
    function pauseTab(){
        ...
        let escape = new Button( (canvas.width/2) - 50, 800, 100, 50, 'pink');
        ...
        ctx.fillText('Escape', canvas.width/2, 830);
